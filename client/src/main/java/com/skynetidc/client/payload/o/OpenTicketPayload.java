package com.skynetidc.client.payload.o;
...
public class OpenTicketPayload extends Payload {
	...
	/***
	 * @param contactID If applicable, the Contact ID to create the ticket for (only if
	 * $clientID is passed).
	 */
	...
	/***
	 * @param serviceID The service to associate the ticket with (only one of $serviceID
	 * or $domainID)
	 */
	...
	/***
	 * @param domainID The domain to associate the ticket with (only one of $serviceID or
	 * $domainID)
	 */
	...
	/***
	 * @param attachments base64 json encoded array of file attachments. Can be the direct
	 * output of a multipart-form-data form submission ($_FILES superglobal in PHP) or an
	 * array of arrays consisting of both a filename and data keys (see example below).
	 */

/**
 * Generated by ramidzkh's awesome generator thingy
 */
package com.skynetidc.client.payload.g;

import com.skynetidc.client.payload.Payload;

/***
 * This class and its methods directly relate to the API documentation. API documentation
 * can be found at the￼ following URL: ￼*
 * https://developers.whmcs.com/api-reference/getmodulequeue/
 */
public class GetModuleQueuePayload extends Payload {

	/*
	 * Obtains the Module Queue for Incomplete Failed Actions
	 */
	public GetModuleQueuePayload() {
		super("GetModuleQueue");
	}

	/***
	 * @param serviceType The type of service to load (‘domain’, ‘service’ or “))
	 */
	public GetModuleQueuePayload withServiceType(String serviceType) {
		put("serviceType", serviceType);

		return this;
	}

	/***
	 * @param moduleName The module name to obtain the queue for in system format. eg
	 * cpanel
	 */
	public GetModuleQueuePayload withModuleName(String moduleName) {
		put("moduleName", moduleName);

		return this;
	}

	/***
	 * @param moduleAction The module action to obtain the queue for. eg CreateAccount,
	 * SuspendAccount
	 */
	public GetModuleQueuePayload withModuleAction(String moduleAction) {
		put("moduleAction", moduleAction);

		return this;
	}

	/***
	 * @param since The date/time since to obtain the items. Format Y-m-d Can include
	 * H:i:s
	 */
	public GetModuleQueuePayload withSince(String since) {
		put("since", since);

		return this;
	}

}

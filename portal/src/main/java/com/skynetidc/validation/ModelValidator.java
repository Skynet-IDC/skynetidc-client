// package com.skynetidc.validation;
//
...
//
// /**
// * This method is to validate model against it's annotation constraints and setting the
// @org.springframework.validation.Error by rejected values.
...
// */
// public void validateModel(final T model, ValidationContext context, final
// Object...args) {
//
// if(model == null) throw new IllegalArgumentException("The model paremter is null");
...
// * and displaying manipulated error messages using that can be sent as a code from
// property file or plain text.
// * This method gives another feature in the message manipulation by replacing all
// attributes that can be sent within the message
// * in curly braces.
// * i.e
// * in property file: min.size.message = The value should not be less than {value}!
// * The message attributes are always expected to be one of annotation attributes such as
...
// */
// public Set<ConstraintViolation<T>> validateModel(final T model, MessageContext
// messages) {
//
// if(model == null) throw new IllegalArgumentException("The model paremter is null");
// if(messages == null) throw new IllegalArgumentException("The messages paremter is
// null");
//
// Set<ConstraintViolation<T>> violations = validator.validate(model);
// if(violations != null && violations.size() > 0) {
...
// try {
// message.append(this.messageSource.getMessage(codeOrMessage, null,
// LocaleContextHolder.getLocale()));
// } catch(NoSuchMessageException e) {
// message.append(codeOrMessage);
// }
//
// // extract attributes from the message if any exist, and replace them by actual values
// in the message
// Matcher m = Pattern.compile( "\\{(.*?)\\}" ).matcher(message);
// while(m.find()) {
// // get actual attribute value in the message
// String attrVal =
// violation.getConstraintDescriptor().getAttributes().getOrDefault(m.group(1),
// m.group()).toString();
//
